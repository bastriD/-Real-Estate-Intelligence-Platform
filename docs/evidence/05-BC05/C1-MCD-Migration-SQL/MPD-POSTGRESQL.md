...
**Source logique :** `MLD-PROJET.md` V4
...
Migration 013 ajoute l'ownership client explicite :

```text
FK fk_demande_client
    id_client
    -> real_estate.client(id_client)
```

`id_client` est `NOT NULL`.

`id_mandat` reste nullable : une demande peut exister avant contractualisation.

La propriété client est indépendante de `demande_affectation` et de l'auteur de `demande_version`.
...
# 21. real_estate.offre

Table physique ajoutée par la migration 014.

| Colonne | Type logique PostgreSQL | Nullabilité / rôle |
| --- | --- | --- |
| id_offre | BIGINT | PK, NON NULL |
| id_presentation | BIGINT | FK, NON NULL |
| numero_version | entier | NON NULL |
| montant | numérique | NON NULL, > 0 |
| date_offre | date/horodatage métier | selon migration 014 |
| date_expiration | date/horodatage métier | optionnel |
| date_decision | date/horodatage métier | optionnel |
| statut | VARCHAR | NON NULL |
| commentaire | texte | optionnel |
| date_creation | horodatage | création |

Relations et invariants vérifiés :
...
UNIQUE
    (id_presentation, numero_version)
...
Le schéma protège également la cohérence des dates de décision / expiration et l'unicité d'une offre acceptée pour une même présentation.

Le versionnement conserve l'historique :
...
Il n'existe pas de FK `offre -> visite`.

La migration 014 n'ajoute pas `vente.id_offre`. Le lien explicite entre une offre acceptée et une vente reste hors de cette migration et ne doit pas être présenté comme implémenté.

Runtime vérifié pour le workflow Offre :
...
Les règles d'ownership CHASSEUR sont vérifiées par tests automatisés. Un E2E runtime direct avec identité CHASSEUR reste non vérifié.

---

# 22. real_estate.vente
...
# 23. real_estate.parametres_honoraires
...
# 24. real_estate.bareme_commission
...
# 25. real_estate.parametres_remuneration
...
# 26. real_estate.palier_performance
...
# 29. Contraintes financières de paiement
...
# 36. Types PostgreSQL structurants
...
# 41. Matrice des clés étrangères
...
# 42. Contraintes UNIQUE structurantes
...
# 43. Intégrité métier portée par PostgreSQL
...
# 44. Calcul financier implémenté
...
# 46. Audit du scénario transactionnel
...
# 47. Relation avec le Data Warehouse
...
# 49. Sécurité physique et logique
...
# 51. Migrations et évolution du MPD
...
# 54. Concepts non présents physiquement
...
# 55. Modèle physique synthétique
...
# Synchronisation migrations 013–014 et runtime

## Migration 013

```text
real_estate.demande.id_client
    -> real_estate.client.id_client
```

Cette évolution ferme le besoin d'ownership client explicite de la demande.

État : `GAP-BUS-001 — CLOSED / RUNTIME VERIFIED`.
...
Cette évolution implémente le workflow d'offre versionné et auditable.

État : `GAP-BUS-002 — CLOSED / RUNTIME VERIFIED WITH RBAC QUALIFICATION`.
...
## Limites à ne pas sur-déclarer

```text
CHASSEUR Offre ownership automated tests
=
VERIFIED
...
OFFRE ACCEPTEE -> VENTE explicit transaction identity
=
NOT IMPLEMENTED / NOT CLAIMED
```

La prochaine migration possible est `015`, uniquement si un nouveau besoin de schéma est démontré.

---

# 57. Conclusion

Le MPD V4 représente désormais le modèle PostgreSQL réellement utilisé par la plateforme.
...
****MPD POSTGRESQL V4 — RUNTIME ALIGNED THROUGH MIGRATION 014****

****MCD V4 → MLD V4 → MPD V4 : SYNCHRONIZED****
