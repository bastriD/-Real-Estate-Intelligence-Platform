...
**Source conceptuelle :** `MCD-MERISE-PROJET.md` V4

**Périmètre :** migrations 001 à 014
...
Le Modèle Logique de Données traduit le MCD V4 en relations relationnelles.
...
Depuis la migration 013, la propriété client est explicite :
...
Cette clé représente le propriétaire métier de la demande. Elle est indépendante de :

```text
id_mandat
DEMANDE_AFFECTATION
auteur de DEMANDE_VERSION
```

`id_mandat` reste nullable afin de conserver le parcours pré-mandat.
...
```text
UNIQUE(id_presentation, numero_version)
numero_version > 0
```
...
Une `PRESENTATION` peut posséder plusieurs versions d'offre.

La révision ne remplace pas la ligne précédente :
...
Une contrainte physique complémentaire garantit qu'une présentation ne possède pas plusieurs offres acceptées simultanément.

Il n'existe pas de FK `OFFRE -> VISITE`.

Il n'existe pas non plus de FK `VENTE -> OFFRE` dans le modèle courant. Le lien transactionnel explicite entre une offre acceptée et une vente reste hors du périmètre de la migration 014.
...
# 33. Éligibilité de rémunération
...
# 38. Graphe relationnel principal
...
# 40. Contraintes d’unicité principales
...
# 43. Séparation des responsabilités
...
DEMANDE.id_mandat / affectation chasseur
...
`OFFRE` n'appartient plus à cette liste : la relation est implémentée depuis la migration 014.

En revanche, une FK transactionnelle explicite `VENTE -> OFFRE` n'est pas implémentée et ne doit pas être revendiquée.
...
Le MLD V4 traduit le MCD V4 dans un modèle relationnel cohérent avec le système actuellement implémenté.
...
## Synchronisation migrations 013 et 014

### Migration 013 — propriété client de la demande

Le MLD rend désormais explicite :
...
Cette relation supporte l'ownership client sans réutiliser le mandat ou l'affectation chasseur comme substitut d'identité.

### Migration 014 — workflow Offre

Le MLD contient désormais :

```text
PRESENTATION 1 -> N OFFRE
```

avec versionnement par `(id_presentation, numero_version)` et cycle de statut.

La migration 014 ne modifie pas `VENTE` pour lui ajouter `id_offre`.
...
CHASSEUR Offre ownership
=
automated tests verified
direct runtime E2E not verified

OFFRE ACCEPTEE -> VENTE explicit identity
=
...
****MLD V4 — ALIGNED WITH MCD V4 AND RUNTIME MODEL THROUGH MIGRATION 012****
